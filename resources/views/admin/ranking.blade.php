    ...
    <main class="admin-main">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
            <div>
                ...
                <p style="color: #64748b; margin-top: 0.5rem;">Clasificación basada en las evaluaciones de los jueces</p>
            </div>
            <a href="{{ route('admin.evaluaciones') }}" class="btn-accion ver" style="padding: 0.75rem 1.5rem; background: #667eea; color: white; text-decoration: none; border-radius: 8px;">
                    ...
                    <tbody>
                        @foreach($ranking as $index => $item)
                            <tr>
                                ...
                                <td>
                                    @php
                                        $porcentaje = $item['total_jueces'] > 0 ? ($item['evaluaciones_recibidas'] / $item['total_jueces']) * 100 : 0;
                                    @endphp
                                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                                        <span style="font-weight: 600; color: {{ $porcentaje == 100 ? '#10b981' : '#f59e0b' }};">
                                            {{ round($porcentaje) }}%
                                        </span>
                                        <div style="flex: 1; max-width: 80px;">
                                            <div class="progress-bar-custom">
                                                <div class="progress-fill" style="width: {{ $porcentaje }}%; background: {{ $porcentaje == 100 ? '#10b981' : '#f59e0b' }};"></div>
                                            </div>
                                        </div>
                                    </div>
                                </td>
