...
class Event extends Model
{
     ...
     * Envía certificados PDF a todos los miembros de los equipos que ocupan los primeros 3 lugares
     * 
     * @param bool $force Si es true, envía certificados incluso si ya fueron enviados previamente
     * @return array Array con estadísticas del envío
     */
    public function enviarCertificadosGanadores($force = false)
    {
        \Log::info("=== INICIANDO ENVÍO DE CERTIFICADOS PARA EVENTO ID: {$this->id} ===");
        
        // Obtener ganadores con manejo de empates
        $ganadores = $this->obtenerGanadoresConEmpates();
        
        $certificadosEnviados = 0;
        $certificadosOmitidos = 0;
        $errores = [];
        
        // Función auxiliar para enviar certificados
        $enviarCertificados = function($ganadores, $lugar) use (&$certificadosEnviados, &$certificadosOmitidos, &$errores, $force) {
            foreach ($ganadores as $ganador) {
                ...
                foreach ($miembros as $miembro) {
                    ...
                    try {
                        // Verificar si ya se envió el certificado por correo (si no es forzado)
                        if (!$force) {
                            ...
                            // Si el certificado existe, asumimos que ya se envió
                            // (se puede mejorar agregando un campo certificado_enviado en el futuro)
                            if ($certificadoExistente) {
                                \Log::info("Certificado ya existe para {$miembro->name}, omitiendo envío...");
                                $certificadosOmitidos++;
                                continue;
                            }
                        }
                        
                        \Log::info("Enviando certificado por correo a {$miembro->email} (ID: {$miembro->id}) para lugar {$lugar}");
                        
                        // Despachar job para enviar certificado
                        \App\Jobs\EnviarCertificadoGanador::dispatch($miembro, $equipo, $this, $lugar, $promedio)
                            ->onQueue('notifications');
                        
                        $certificadosEnviados++;
                        \Log::info("Job de envío de certificado despachado para {$miembro->email}");
                        
                    } catch (\Exception $e) {
                        $errorMsg = "Error enviando certificado a {$miembro->email}: " . $e->getMessage();
                        $errores[] = $errorMsg;
                        \Log::error($errorMsg);
        ...
        // Enviar certificados a primer lugar
        if (!empty($ganadores['primer_lugar'])) {
            $enviarCertificados($ganadores['primer_lugar'], 1);
        }
        
        // Enviar certificados a segundo lugar
        if (!empty($ganadores['segundo_lugar'])) {
            $enviarCertificados($ganadores['segundo_lugar'], 2);
        }
        
        // Enviar certificados a tercer lugar
        if (!empty($ganadores['tercer_lugar'])) {
            $enviarCertificados($ganadores['tercer_lugar'], 3);
        }
        
        \Log::info("=== FINALIZADO ENVÍO DE CERTIFICADOS. Enviados: {$certificadosEnviados}, Omitidos: {$certificadosOmitidos} ===");
        
        return [
            'certificados_enviados' => $certificadosEnviados,
            'certificados_omitidos' => $certificadosOmitidos,
            'errores' => $errores,
